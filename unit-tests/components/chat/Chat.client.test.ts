...
describe('app/components/chat/Chat.client.tsx', () => {
  ...
  it('first send appends user message and clears draft input immediately', async () => {
    ...
    expect(screen.getByTestId('draft-input').textContent).toBe('seed prompt');
    ...
    const appendPayload = appendMock.mock.calls[0][0];
    expect(appendPayload.role).toBe('user');
    expect(appendPayload.content).toContain('hello world');
