...
describe('app/components/chat/ChatBox.tsx', () => {
  it('shows Workbench toggle next to mode selector when chat has started', () => {
    showWorkbenchValue = false;

    render(<ChatBox {...buildBaseProps()} />);

    expect(screen.getByTitle('Show Workbench')).toBeTruthy();
    expect(screen.getByTitle('Discuss')).toBeTruthy();
  });

  it('toggles workbench visibility when Workbench button is clicked', () => {
    showWorkbenchValue = false;
    setShowWorkbenchMock.mockReset();

    render(<ChatBox {...buildBaseProps()} />);

    fireEvent.click(screen.getByTitle('Show Workbench'));

    expect(setShowWorkbenchMock).toHaveBeenCalledWith(true);
  });
});
