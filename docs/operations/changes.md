ja# File and Folder Locking Feature Implementation
...
## Agent Run Persistence Parity (SQLite + PostgreSQL/PostgREST)

- Added provider-agnostic agent-run persistence routing through `app/lib/.server/persistence/index.ts`.
- Added PostgREST persistence operations for agent runs in `app/lib/.server/persistence/postgrest-memory.ts`:
  - `upsertAgentRunRecord`
  - `readAgentRunRecord`
  - `listAgentRunRecords`
- Updated `AgentRunService` to use the shared persistence facade instead of direct SQLite import.
- Updated external PostgreSQL schema in `docs/postgrest-schema.sql` with:
  - `agent_runs` table
  - `idx_agent_runs_updated_at` index
- SQLite already includes equivalent `agent_runs` table in `app/lib/.server/persistence/sqlite-memory.ts`, so both backend variants now support the same orchestration persistence feature.

---

## OpenClaw + Platform/Core Foundation Cycle

- Added OpenClaw adapter module:
  - `app/lib/.server/extensions/openclaw/openclaw-client.ts`
  - execute / cancel / status helpers with env-driven base URL
- Completed `api.agent-runs` OpenClaw integration:
  - persisted list/get loader paths
  - remote status enrichment for OpenClaw runs
  - remote cancel delegation for OpenClaw run cancellation
  - model-engine routing via new core router
- Added core layer foundations:
  - `app/core/model-router.ts`
  - `app/core/chat-engine.ts`
- Added platform layer foundations:
  - rate-limit module (`app/platform/security/rate-limit.ts`)
  - authz module (`app/platform/security/authz.ts`)
  - request context + error response helpers (`app/platform/http/*`)
  - request guard (`app/platform/security/request-guard.ts`)
  - persistence schema version module (`app/platform/persistence/schema-version.ts`)
- Added schema version metadata write on SQLite startup (`schema_meta` table).
- Added observability endpoint:
  - `GET /api/metrics`
- Enhanced `GET /api/health`:
  - request-id header/body
  - startup persistence check status
- Added required docs:
  - `INSTALL.md`
  - `API.md`
  - `docker-compose.production.yaml`
  - architecture updates in `README.md` and `PROJECT.md`

---

## Docs Tree Completion + MkDocs Navigation

- Completed requested docs hierarchy under `docs/docs`:
  - `setup/docker/*`
  - `setup/single-host/*`
  - `architecture/*`
  - `database/*`
  ...
  - `development/*`
  - `operations/*`
- Updated `docs/mkdocs.yml` with full `nav` tree so all created pages are visible in docs site navigation.
- Preserved external-service connector model documentation:
  - PostgreSQL/PostgREST external
  - OpenClaw external
  - app remains deployable without bundling those services.

Validation:

- Before docs completion run: `pnpm run test:unit` -> **290 files / 503 tests passed**
- After docs completion run: `pnpm run test:unit` -> **290 files / 503 tests passed**
